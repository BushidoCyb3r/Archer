...
func TestCorrelate_NoDuplicateWhenFreshSensorResolvedAtEmit(t *testing.T) {
	...
	a.SetFindingsProvider(&stubFindingsProvider{findings: []model.Finding{
		{ID: 187626, Type: "Off-Hours Transfer", SrcIP: "10.18.61.48", DstIP: "35.163.162.183", Score: 52, Sensor: "deathstar", Timestamp: "2026-05-10 02:03:34 UTC"},
	}})
...
func TestAnalyzerAdd_ResolvesSensorFromSourceFile(t *testing.T) {
	...
	a.add(model.Finding{
		...
		SrcIP:      "10.0.0.1",
		DstIP:      "1.2.3.4",
		SourceFile: "/logs/deathstar/2026-05-10/conn.log",
	})
	if got := a.findings[0].Sensor; got != "deathstar" {
		t.Errorf("Sensor = %q; want \"deathstar\" resolved from SourceFile", got)
	}
}
