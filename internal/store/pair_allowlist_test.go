...
func TestPairAllowlist_MatchScopeAndReload(t *testing.T) {
	...
	if _, err := s1.AddPairAllow(model.PairAllowEntry{
		Src: "10.0.1.1", Dst: "2.2.2.2", Port: "443", FindingType: "Beaconing",
		Sensor: "sensorA",
	}); err != nil {
		t.Fatalf("AddPairAllow sensor-scoped: %v", err)
	}
	if !s1.IsPairAllowed("10.0.1.1", "2.2.2.2", "443", "Beaconing", "sensorA") {
		t.Error("sensor-scoped rule should hide its own sensor")
	}
	if s1.IsPairAllowed("10.0.1.1", "2.2.2.2", "443", "Beaconing", "sensorB") {
		t.Error("sensor-scoped rule should not hide a different sensor")
	}
	if s1.IsPairAllowed("10.0.1.1", "2.2.2.2", "443", "Beaconing", "") {
	...
	if !s2.IsPairAllowed("10.0.1.1", "2.2.2.2", "443", "Beaconing", "sensorA") {
		t.Error("sensor-scoped rule did not survive reload")
	}
	if s2.IsPairAllowed("10.0.1.1", "2.2.2.2", "443", "Beaconing", "sensorB") {
