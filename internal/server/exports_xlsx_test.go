...
func TestBuildCampaignsRollup_FiltersByDistinctSources(t *testing.T) {
	findings := []model.Finding{
		...
		{Type: "Beacon", SrcIP: "192.168.1.5", DstIP: "198.51.100.10", DstPort: "443", Score: 80},
		...
		{Type: "Beacon", SrcIP: "192.168.1.5", DstIP: "203.0.113.5", DstPort: "80", Score: 70},
		// (network) is excluded
...
func TestBuildHostsRollup_OrgIPOnly(t *testing.T) {
	findings := []model.Finding{
		...
		{Type: "Beacon", SrcIP: "192.168.1.5", DstIP: "1.2.3.4", Score: 80, Severity: model.SevCritical},
		// Public src IP without an admin CIDR — excluded
		{Type: "Beacon", SrcIP: "8.8.8.8", DstIP: "1.2.3.4", Score: 50, Severity: model.SevMedium},
...
func TestWriteFindingsSheet_RoundTripsViaExcelize(t *testing.T) {
	...
	defer xf.Close()
	findings := []model.Finding{
		{Score: 95, Severity: model.SevCritical, Type: "Beacon",
	...
	if val != "Beacon" {
		t.Errorf("C2 = %q, want %q (type column)", val, "Beacon")
	}
