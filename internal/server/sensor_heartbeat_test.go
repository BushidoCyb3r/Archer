...
func TestSensorsHealthEndpoint(t *testing.T) {
	...
	for _, h := range payload.Sensors {
		...
		if h.StaleThresholdSec != int64(sensorStaleThreshold.Seconds()) {
			t.Errorf("%s.stale_threshold_sec = %d, want %d", h.Name, h.StaleThresholdSec, int64(sensorStaleThreshold.Seconds()))
		}
	}
