...
`status`, `done`, `notification`, `ti_result`, `ti_done`, `llm_done`,
...
| `GET` | `/api/findings/trend` | any | `{days, series, severity_series}` — per-UTC-day finding counts over a contiguous zero-filled day axis (bucketed on the event `timestamp` date), grouped two ways in one pass: `series` rolls finding types into seven detection families (beaconing / ti / exfil / dns / lateral / tls / other), `severity_series` buckets by severity tier. Empty-in-range groups are omitted; roll-up types are excluded. Accepts the same filter surface as `GET /api/findings` (including `status` and `delta`; `limit`/`offset`/`sort`/`dir` are ignored). Powers the trend chart above the findings table. |
...
| `POST` | `/api/findings/bulk` | analyst+ | Batch status transition. Body `{action: "ack"\|"esc"\|"dismiss"\|"open", ids?: [int], note?: string}`. When `ids` is omitted, the target set is every finding matching the request's filter query params (the same surface as `GET /api/findings` — the "select all N matching" path). Applies in one transaction; returns `{affected, prior}` where `prior` is the per-finding `[{id, status}]` snapshot the undo toast replays. `esc` flips status and SIEM-forwards each finding (same side effect as the single escalate) but does **not** run TI vendor enrichment. |
...
| `POST` | `/api/findings/{id}/enrich` | analyst+ | Run AI triage on the finding (requires AI enrichment enabled+configured, else 400). Redacts internal addresses, asks the configured LLM provider for a verdict-first briefing, appends it as an `AI Triage` note, and emits `llm_done` on completion. Annotation-only — never changes score/severity/status. |
...
- `correlations` is the list of sibling finding IDs that share this
  ...
  Activity roll-up. The roll-up is a separate `Correlated Activity`
  finding; the SPA pivots to the pair via right-click → **Show
  contributing activity** on that row.
...
| Method | Path | Role | Notes |
|--------|------|------|-------|
| `GET` | `/api/allowlist` | any | Returns a bare JSON array of strings — `["1.2.3.4", "# infrastructure", "10.0.0.0/8", "*.in-addr.arpa", …]`. Entries match a finding's source/destination as an exact value, a CIDR range, or a `*`/`?` wildcard glob (anchored, case-insensitive — `*.in-addr.arpa`, `185.220.*`); CIDR is tried first, so `10.0.0.0/8` stays a CIDR. `#` lines are comments. |
...
| `GET` | `/api/attack-coverage` | any | MITRE ATT&CK coverage over the current findings, powering the **ATT&CK Coverage** modal. `{"techniques":[{"id","name","tactic","url","count","types":[{"type","count"}]}],"unmapped":[{"type","count"}],"total"}`. Techniques sorted by count desc; a finding whose type maps to N techniques counts toward all N (coverage, not a partition). `unmapped` is the finding types carrying no ATT&CK technique (TI hits, roll-ups, Zeek notices). The finding-type → technique map is also bootstrapped into the page as `window.ATTACK_MAP` for the detail-pane technique chips. |
...
| `GET` | `/api/llm/status` | any | AI enrichment status: `{enabled: bool, provider: string, local: bool}`. `enabled` is true only when enrichment is on **and** the provider settings validate; `local` is true for on-network/enclave providers (`ollama`/`dod`/`custom`). No secrets exposed. The UI uses it to show/hide the **AI Triage** button + right-click item and label its egress posture. The enrich action itself is `POST /api/findings/{id}/enrich` (see Findings). |
