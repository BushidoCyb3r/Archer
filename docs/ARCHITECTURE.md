...
| 2.5 | SNI/JA3/JA4 enrichment for conn Beaconing — deferred here so `sslUIDIndex` is fully populated before the lookup. Also stamps a **TLS-fingerprint rarity + cross-host-cluster** note (enrichment only, no score change) from the per-fingerprint prevalence map `analyzeSSL` built over all `ssl.log`. Then DGA hostname augmentation: sweep over Beaconing / HTTP Beaconing findings; bumps score (+15) and severity when the destination Hostname's SLD looks algorithmically generated (high entropy + low English bigram log-likelihood). | `analyzer.go: enrichBeaconSNI`, `dga.go: applyDGAScoring` |
