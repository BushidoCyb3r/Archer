...
- **Enrollment with a malformed public key**: HTTP 400. The submitted `pubkey` must be a recognized key type (`ssh-ed25519`, `ssh-rsa`, the `ecdsa-sha2-*` set, the `sk-*` security-key types) followed by a base64-decodable blob; the server rejects anything else at the boundary rather than writing an `authorized_keys` line sshd would silently refuse at connect time (which would leave a live-but-unusable sensor row).
- **Enrollment with a key already in use**: HTTP 409. Each sensor needs its own keypair. sshd matches by key and runs the *first* matching `authorized_keys` line's forced `command="rrsync -wo /logs/<name>/"`, so two sensors sharing one key would land the second's pushes in the first's log directory. The server rejects a key already bound to an active sensor and rolls the consumed enrollment token back so the operator can retry with a distinct key.
