...
| `ai` | Has an AI-triage briefing | `ai:true` matches findings that carry an AI Triage note (you ran — or auto-ran on escalate — the AI enrichment on them); `ai:false` excludes them. E.g. `severity:critical AND ai:false` finds high-priority findings still awaiting an AI pass. |
